...
class PagererPresetEditForm extends PagererPresetFormBase {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    foreach (['left', 'center', 'right'] as $pane) {
      ...
      // Check that the PagererStyle exists, fall back to 'basic' if not.
      $style = $this->entity->getPaneData($pane, 'style');
      if ($style !== 'none' && !$this->styleManager->hasDefinition($style)) {
        $this->messenger->addMessage($this->t('The %style pager style is not available.', ['%style' => $style]), 'error');
        $style = 'basic';
      }
   ...
   */
  public function processStyleChange(array $form, FormStateInterface $form_state): AjaxResponse {
