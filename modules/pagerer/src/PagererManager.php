...
class PagererManager extends PagerManager {
   ...
   * @var bool
   */
  protected bool $querystringOverride;
   ...
   *   The pager index.
   */
  protected function setPagerer(Pagerer $pager, int $element = 0): void {
   ...
   */
  public function getPagererUpdatedParameters(Pagerer $pager, array $parameters, int $page, array $adaptive_keys = []): array {
   ...
   *   configuration.
   */
  protected function adaptiveKeysToUrl(array $ak, int $last_page): ?string {
    if (empty($ak)) {
      return NULL;
    }
   ...
   */
  public function getHref(Pagerer $pager, array $parameters, int $page, array $adaptive_keys = [], bool $set_query = TRUE): Url {
    $options = $set_query ? [
