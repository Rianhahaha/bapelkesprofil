...
class PagererParameters extends PagerParameters {
   ...
   */
  protected function getPagererItem(string $item_id): ?array {
   ...
   */
  protected function parsePagerer(string $hash, string $querystring): void {
   ...
   */
  protected function parseLegacy(string $hash, string $page, string $page_ak): void {
    // Current pages.
    if (!empty($page)) {
      $tmp = explode(',', $page);
      foreach ($tmp as $pager_id => $page) {
